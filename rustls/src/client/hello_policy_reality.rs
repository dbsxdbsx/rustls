...
pub struct RealityHelloPolicy {
    ...
    /// REALITY加密配置（可选，用于动态生成令牌）
    #[cfg(feature = "reality-crypto")]
    pub crypto_config: Option<RealityConfig>,
}
...
impl RealityHelloPolicy {
    ...
    pub fn with_reality_token(mut self, token: Vec<u8>) -> Self {
        if token.len() > 32 {
            panic!("REALITY token too long: {} bytes (max 32)", token.len());
        }
        self.reality_token = Some(token);
        self
    }

    /// 设置REALITY加密配置（用于动态生成令牌）
    #[cfg(feature = "reality-crypto")]
    pub fn with_crypto_config(mut self, config: RealityConfig) -> Self {
        self.crypto_config = Some(config);
        self
    }
    ...
    fn inject_reality_token_with_token(
        ...
        self.inject_token_into_clienthello(client_hello_bytes, token)
    }

    /// 将REALITY令牌注入到ClientHello的legacy_session_id字段（向后兼容）
    fn inject_reality_token(&self, client_hello_bytes: &[u8]) -> Option<Vec<u8>> {
        let token = self.reality_token.as_ref()?;
        self.inject_reality_token_with_token(client_hello_bytes, token)
    }

    /// 实际的令牌注入逻辑
    fn inject_token_into_clienthello(
        &self,
        client_hello_bytes: &[u8],
        token: &[u8],
    ) -> Option<Vec<u8>> {
        ...
        // 替换session_id数据
        let mut new_bytes =
            Vec::with_capacity(modified_bytes.len() - session_id_len as usize + token.len());
...
impl HelloPolicy for RealityHelloPolicy {
    ...
    fn reality_inject_clienthello(
        &self,
        original_bytes: &[u8],
        _ctx: &HelloPolicyContext<'_>,
    ) -> Option<Vec<u8>> {
        // 首先尝试使用预配置的令牌
        if let Some(token) = &self.reality_token {
            return self.inject_reality_token_with_token(original_bytes, token);
        }

        // 如果没有预配置令牌，尝试动态生成
        #[cfg(feature = "reality-crypto")]
        if let Some(config) = &self.crypto_config {
            match self.generate_dynamic_token(config) {
                Ok(token) => return self.inject_reality_token_with_token(original_bytes, &token),
                Err(_e) => {
                    // 记录错误但不中断连接
                    #[cfg(feature = "log")]
                    log::warn!("Failed to generate REALITY token: {}", _e);
                    return None;
                }
            }
        }
...
mod tests {
    ...
    fn test_reality_token_injection() {
        let policy =
            RealityHelloPolicy::new().with_reality_token(b"test_reality_token_123456".to_vec());
        ...
        let mut mock_client_hello = vec![
            // TLS记录头 (5 bytes)
            0x16, 0x03, 0x01, 0x00,
            0x00, // ContentType=Handshake, Version=TLS1.0, Length=待更新
            // 握手头 (4 bytes)
        ...
        assert_eq!(
            modified[5 + 4 + 2 + 32],
            b"test_reality_token_123456".len() as u8
        );
        ...
        assert_eq!(
            &modified[session_id_start..session_id_end],
            b"test_reality_token_123456"
        );
    }
    ...
    fn test_cipher_suite_reordering() {
        ...
        assert_eq!(reordered[1], CipherSuite::TLS13_AES_256_GCM_SHA384);
        assert_eq!(reordered[2], CipherSuite::TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256);
    }
}
